...
| -------- | ------------- | ---------------------- |
| มืดสนิท  | 200 kΩ – 2 MΩ |         3.369         |
| ห้องปกติ | 10 kΩ – 50 kΩ |         2.429         |
| ใต้โคมไฟ | 2 kΩ – 10 kΩ  |                      |
| กลางแจ้ง | 200 Ω – 2 kΩ  |        1.767          |
...
<img width="750" height="300" alt="image" src="https://github.com/user-attachments/assets/8013de1f-3ebf-4ff1-8dc7-c03f39018680" />
...
1. ทำไมต้องใช้ Voltage Divider ในการอ่าน LDR?
   LDR เปลี่ยนแค่ค่าความต้านทาน แต่ ADC อ่านได้เฉพาะแรงดันไฟฟ้า  Voltage Divider เพื่อเปลี่ยนค่าความต้านทานให้เป็นแรงดันที่นำไปวัดได้
2. ทำไมต้องเก็บข้อมูลหลายจุดก่อนทำ Calibration?
   ค่าที่วัดได้แต่ละครั้งอาจไม่เท่ากัน การเก็บหลาย ๆ ค่าแล้วนำมาเฉลี่ยจะช่วยลดความผิดพลาด
3. ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น?
   แยกไม่ออกว่าค่าไหนเป็นสัญญาณจริง ทำให้ระบบทำงานผิดพลาด
4. ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity?
   วางไม่เหมาะสม เซนเซอร์อาจรับสัญญาณรบกวนหรือวัดค่าไม่ตรง
5. ถ้า ADC มี Resolution ต่ำ จะส่งผลอย่างไรต่อความแม่นยำ?
   ADC จะอ่านค่าได้ไม่ละเอียด การเปลี่ยนแปลงเล็ก ๆ ของสัญญาณ
...
| --- | -------------------------------------------------------------- | -------------------- | ------ |
| 1.1 | อธิบายหลักการทำงานของ LDR ได้ถูกต้อง                           |      3     | เข้าใจหลักการทำงาน  |
| 1.2 | อธิบายการทำงานภายใน LDR (โฟตอน → อิเล็กตรอน → conduction band) |    4    |  เข้าใจหลักการ แต่ยังอธิบายเชิงลึกไม่ครบ      |
| 1.3 | เข้าใจว่าความสัมพันธ์ R vs Lux ไม่เป็นเส้นตรง                  |      4       | เข้าใจภาพรวม แต่ยังอธิบายเหตุผลได้ไม่ละเอียด       |
| 1.4 | เข้าใจเหตุผลที่ต้องใช้ Voltage Divider                         |      4     |  อธิบายพอสังเขป      |

### **ส่วนที่ 10.2 — การต่อวงจรและอ่านค่า ADC (20 คะแนน)**

| ข้อ | รายการ                              | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ----------------------------------- | -------------------- | ------ |
| 2.1 | ต่อวงจร Voltage Divider ได้ถูกต้อง  |           5           |  ต่อวงจรและใช้งานได้      |
| 2.2 | เลือกค่า R คงที่เหมาะสม (เช่น 10kΩ) |             5         | ใช้ตัวต้านทาน 10kΩ เหมาะสม       |
| 2.3 | เขียนโปรแกรมอ่านค่า ADC ได้ถูกต้อง  |              4        | โปรแกรมทำงานได้ แต่ต้องแก้ไขเล็กน้อยก่อนใช้งาน    |
| 2.4 | ใช้ ESP‑IDF ADC + Calibration ได้   |       4        | ใช้งานได้ แต่ยังไม่เข้าใจการ Calibration ทั้งหมด       |

### **ส่วนที่ 10.3 — การเก็บข้อมูลจริง (20 คะแนน)**

| ข้อ | รายการ                             | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ---------------------------------- | -------------------- | ------ |
| 3.1 | เก็บข้อมูล ADC อย่างน้อย 5–7 จุด   |          5            | เก็บข้อมูลครบ       |
| 3.2 | วัดค่า Lux จากมือถือได้ถูกต้อง     |           4          | วัดได้ แต่ค่ามีความคลาดเคลื่อนเล็กน้อย       |
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน           |         5             | บันทึกข้อมูลครบ       |
| 3.4 | ควบคุมสภาพแสงให้คงที่ระหว่างการวัด |           3           |  ควบคุมแสงได้ไม่คงที่ตลอดการทดลอง      |
...
| ข้อ | รายการ                                        | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | --------------------------------------------- | -------------------- | ------ |
| 4.1 | สร้างกราฟ Scatter ADC vs Lux ได้              |         4             |   สร้างกราฟ     |
| 4.2 | เลือก Trendline เหมาะสม (Linear / Polynomial) |        4             |  เลือกได้       |
| 4.3 | อ่านสมการจากกราฟได้ถูกต้อง                    |          4            |อ่านสมการได้ มีผิดพลาดเล็กน้อย        |
| 4.4 | นำสมการไปใช้ในโค้ดได้                         |         4            | ใช้งานได้ แต่ต้องปรับค่าบางส่วน       |
| 4.5 | วิเคราะห์ว่า Linear หรือ Polynomial เหมาะกว่า |            4          |อธิบายได้ แต่ยังไม่ละเอียด        |
### **ส่วนที่ 10.5 — การทดสอบผลลัพธ์และวิเคราะห์ (15 คะแนน)**

| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ---------------------------------------- | -------------------- | ------ |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล |           4           |   มีการทดสอบเพิ่มเติม     |
| 5.2 | เปรียบเทียบ Lux มือถือ vs Lux จากโค้ด    |        4             | เปรียบเทียบได้ แต่ยังมีค่าคลาดเคลื่อน       |
| 5.3 | วิเคราะห์ความคลาดเคลื่อนและสาเหตุได้     |          3          |วิเคราะห์สาเหตุได้บางส่วน แต่ยังไม่ครบทุกปัจจัย        |
.................................................................
...
1. จุดไหนในใบงานที่คิดว่ายากที่สุด? 
   วัดค่าแสง คาดเคลื่อนผลการทดลองจากการควบคุมแสง     
